...
public float getAdjustedPosition(float onCoordinate, float offCoordinate, float onVal, float offVal){
...
public void drawCountour(int i, int j, int xPos, int yPos, int state){
...
public float getDistanceFromCircle(int x, int y){
  ...
  for(int i = 0; i < circles.length; i++){
    ...
    maxDist += PApplet.parseFloat(circles[i].radius * circles[i].radius) / max(1.0f, (xDistance * xDistance + yDistance * yDistance));
  }
  return maxDist;
}

public int getStateFromCorners(float a, float b, float c, float d){
...
public void setup(){
  ...
  for(int i = 0; i < circles.length; i++){
    circles[i] = new Circle(PApplet.parseInt(random(100, width - 100)), PApplet.parseInt(random(100, height - 100)), //xPos, yPos
                            PApplet.parseInt(random(20, 80)), random(0.1f, 2.5f), random(0.1f, 2.5f)); //radius, xSpeed, ySpeed
    circles[i].hitBottom = (random(1) >= 0.5f) ? true : false;
    circles[i].hitRight = (random(1) >= 0.5f) ? true : false;
  }
}
